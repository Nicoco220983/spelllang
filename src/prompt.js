/**
 * @fileoverview Dynamic LLM prompt generator tailored to caller application schema and state definitions.
 */

/**
 * @typedef {Object} FunctionDoc
 * @property {string} name Function name
 * @property {string} [signature] Function signature (e.g. "getSensors(type: Str): List<Map>")
 * @property {string} [description] What the function does
 */

/**
 * @typedef {Object} PromptSchema
 * @property {string} [title] Optional custom guide title
 * @property {Array<FunctionDoc|string>} [functions] Host functions available to the script
 * @property {Object<string, string>} [state] Description of known persistent state properties
 * @property {string} [instructions] Additional domain instructions
 */

/**
 * Generates an LLM system prompt for spelllang based on host schema.
 * @param {PromptSchema} [schema]
 * @returns {string} Formatted Markdown prompt
 */
export function generatePrompt(schema = {}) {
    const title = schema.title || 'spelllang Generation Guide';
...
- \`find(list: List, predicate: Fn): Any\` (returns null if not found)
...
`;
    ...
    if (funcs.length > 0) {
        prompt += `\n## Host Environment Functions\nThe following host functions are injected directly into the script scope:\n`;
    ...
    // Add State Schema
    if (schema.state && Object.keys(schema.state).length > 0) {
        prompt += `\n## Persistent State Schema (\`state.\`)\n`;
        for (const [key, desc] of Object.entries(schema.state)) {
            prompt += `- \`state.${key}\`: ${desc}\n`;
        }
    }
